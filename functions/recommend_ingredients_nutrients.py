...
def ingredient_recommendation(ingredient_models,breed_size, age_type_categ,disorder_type, selected_disorder,vectorizer,svd,encoder, df_standart):
   ...
   additional_ingrs=sorted([i for i in top_ings], key=lambda x: x[1], reverse=True)[:5]	
   additional_ingrs= [i[0] for i in additional_ingrs]
   additional_ingrs=[i for i in additional_ingrs if i not in [pr,c_c,c_v,f] and i in df_standart["name_feed_ingredient"].tolist()]
